...
**Idempotency keys.** The API accepts an optional `idempotency-key` on consent, payment and refund creation and replays a retried request with the same key and payload instead of creating twice. On consent and payment creation that replay lasts 24 hours; a refund key carries no such window, staying bound to the refund it created, and a replayed refund returns that refund's original `refund_id`. This SDK makes the key **required** on every consent and payment create call (`createQuickPayment`, `createSingleConsent`, `createEnduringConsent`, `createPayment`, `createFixedRecurringPayment` and their typed helpers) and **optional but recommended** on the refund helpers. That is the SDK's choice, not the API's: a checkout that can be retried by the browser, a queue or a network blip should never be able to debit twice. Use a fresh UUID for each distinct operation, persist it with the order, and reuse it only when retrying that same request. Subscriptions take no key; list existing subscriptions before retrying a failed create by hand.
...
| `409` | `BP703` / `BP708` | Payment idempotency key reused while the first payment request is still in flight | Poll the named payment; do not resubmit |
...
| `409` | `BP711` | Refund idempotency key reused while the first refund request is still in flight | Retry once that request has completed |
...
### Refunds
How the payment settled decides the refund type, so store the settled payment's `accepted_reason` (`Enum\AcceptedReason`) at payment time. The API accepts an optional idempotency key on refunds: retrying with the same key and the same payload replays the original `201` with the original `refund_id`, so the retry reconciles against the refund already created rather than making a second one. The same key with a different payload is rejected with `409 BP702`; a request arriving while another holding that key is still in flight gets `409 BP711`, which you retry once the first has completed. The helpers take the key as an optional argument, and money-moving refunds should always send one: several refunds may be made against one payment up to its total, so a blind retry of a keyless refund can refund the customer twice. Without a key, the SDK does not retry a refund on a `5xx` or transport failure, since the first attempt may have gone through.
